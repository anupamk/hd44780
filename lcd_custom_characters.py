...
def load_custom_character(custom_char, force = False):
    num_cc = len(load_custom_character)
    if ((force == False) and
    ...
    if (cc_location == None):
        lcd_driver.exec_named_cmd('DISPLAY_ON_CURSOR_OFF')

        # create-it
        custom_char.cgram_location = num_cc + 1
        create_custom_charset(custom_char.cgram_location, custom_char.byte_seq)

        # add it to the available list
        load_custom_character[custom_char.name] = custom_char
        
        lcd_driver.exec_named_cmd('DISPLAY_ON_CURSOR_ON_BLINK_ON')
...
def cc_printf(row, col, shape_name = None):
    ...
    lcd_driver.write_custom_character_at(row, col, cc_cgram_loc)
